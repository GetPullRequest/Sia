...
export async function getGitCredentials(params: {
  ...
  if (!installationId) {
    ...
    const needsRefresh = tokenMissing || expiresIn === 0 || 
...
export async function getVibeCoderCredentials(params: {
  ...
  if (params.agentId) {
    ...
    if (agent[0]) {
      ...
      let decryptedExecutablePath: string | undefined = executablePath || undefined;
      
      // If integration found and has accessToken, decrypt it to get executable path
      if (integration[0]?.accessToken) {
        const metadata = integration[0].metadata as Record<string, unknown> | null;
        ...
        if (storageType) {
          try {
            ...
            console.error(`Failed to decrypt vibe-agent executable path: ${error}`);
  ...
  throw new Error('Agent not found or no agentId provided. Cannot determine vibe-agent credentials.');
}
