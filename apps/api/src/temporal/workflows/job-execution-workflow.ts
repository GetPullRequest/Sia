...
export async function jobExecutionWorkflow(params: {
  ...
  try {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Unknown error';
      verificationErrors.push(`Validation failed: ${errorMsg}`);
    }

    // Step 8: Create PRs (even if Build/Verify failed) - PR body will be generated by agent based on changes
        ...
        try {
          const repoPrResult = await sendCommandToAgent({
            ...
            payload: {
              ...
              title: `Auto-generated PR for job ${jobId}`, // Will be overridden by agent if generated
              body: '', // Will be generated by agent based on changes
              verificationErrors: verificationErrors,
              vibeCoderCredentials: {
                type: vibeCoderCredentials?.type,
                executablePath: vibeCoderCredentials?.executablePath,
                apiKey: vibeCoderCredentials?.apiKey,
                githubToken: gitCredentials?.token,
                githubUsername: gitCredentials?.username,
              },
              agentId,
            },
          });
