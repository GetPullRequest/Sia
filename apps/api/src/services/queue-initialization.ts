...
export async function initializeQueueWorkflows(): Promise<void> {
  try {
    ...
    if (agents.length === 0) {
      console.log('⚠️  No active agents found, skipping schedule initialization');
      console.log('   To create schedules, create an agent with status="active" or update an existing agent to active');
      return;
    }

    console.log(`📅 Initializing queue schedules for ${agents.length} active agent(s)...`);

    for (const agent of agents) {
      try {
        console.log(`   Creating schedule for agent: ${agent.id} (org: ${agent.orgId})`);
        await initializeScheduleForAgent(agent.id, agent.orgId);
        console.log(`   ✅ Schedule created for agent: ${agent.id}`);
      } catch (error) {
        console.error(`   ❌ Failed to initialize schedule for agent ${agent.id}:`, error);
...
export async function initializeScheduleForAgent(
  ...
  try {
    ...
    if (!errorMsg.includes('already exists') && !errorMsg.includes('AlreadyExists')) {
      console.error(`   Failed to create schedule for agent ${agentId}:`, errorMsg);
      throw error;
    }
  }
}
