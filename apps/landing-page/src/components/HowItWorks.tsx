...
const steps = [
  {
    ...
    title: 'Assign',
    description:
      'Before you clock out, assign coding tasks to Sia via Slack or Discord. Simple, natural language works.',
  ...
  },
  {
    ...
    title: 'Sleep',
    description:
      "While you're sleeping, Sia uses AI coding tools to implement your task, run tests, and create a pull request.",
    example: [
      '// Sia working...',
      '✓ Code written',
      '✓ Tests passed',
  ...
  },
  {
    ...
    title: 'Review',
    description:
      'Wake up to a notification. Review the PR on mobile, approve it, or request changes. Ship faster.',
    example: [
...
export default function HowItWorks() {
  ...
  return (
    <section
      id="how-it-works"
      className="py-24 bg-background overflow-hidden w-full max-w-full"
    >
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto relative">
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={index}
                className={`bg-card rounded-2xl p-8 border transition-all duration-1000 group relative ${
                  isActive
              ...
              >
                <div
                  className={`absolute -top-4 -left-4 w-12 h-12 rounded-full flex items-center justify-center font-bold text-xl shadow-lg transition-all duration-1000 ${
                    isActive
                ...
                {index < steps.length - 1 && (
                  <div className="hidden md:block absolute -right-4 top-1/2 -translate-y-1/2 z-10">
                    <div
                      className={`w-8 h-0.5 transition-all duration-1000 ${
                ...
                <div
                  className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-all duration-1000 ${
                    isActive
                      ? 'bg-gradient-to-br from-purple-600 to-green-600 scale-110 rotate-0'
                      : 'bg-gray-100 scale-100 rotate-12'
                  }`}
                >
                  <step.icon
                    className={`w-8 h-8 ${
                      isActive ? 'text-text' : 'text-black'
                    }`}
                  />
                </div>
                ...
                <p
                  className={`text-base font-extralight text-text/60 leading-relaxed mb-6 transition-all duration-1000 ${
                ...
                <div
                  className={`bg-background rounded-xl p-4 border border-white/10 overflow-hidden transition-all duration-1000 ${
                    isActive
                ...
                >
                  {Array.isArray(step.example) ? (
                    ...
                    <code
                      className={`font-mono text-sm block transition-all duration-500 ${
                        isActive
                          ? 'text-text/80 opacity-100 translate-x-0'
                          : 'text-text/50 opacity-80 translate-x-0'
                      }`}
