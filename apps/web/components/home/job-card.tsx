...
export function JobCard({
  ...
  const jobStack = searchParams.get('jobStack')?.split(',').filter(Boolean) || []
  ...
  const openModal = () => {
    ...
    const currentStack = params.get('jobStack')?.split(',').filter(Boolean) || []
    ...
    >
      <CardHeader className="pb-3 px-4 pt-4">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-start gap-2 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1.5">
                <div className={cn('h-2 w-2 rounded-full flex-shrink-0', statusColors[job.status] || 'bg-gray-400')} />
                <button
          ...
          {job.status !== 'failed' && job.status !== 'in-review' && job.status !== 'completed' && (
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {!hideActions && (
            <div className="flex items-center gap-0.5">
              ...
              {job.status === 'in-progress' && onStart && hasStoppedAnimation && (
                <Button
                  size="sm"
                  variant="ghost"
                  className="h-7 px-2 text-xs hover:bg-accent/50 transition-colors"
              ...
              {job.status === 'in-progress' && (onCancel || onStop) && !hasStoppedAnimation && (
                <Button
                  size="sm"
                  variant="ghost"
                  className="h-7 px-2 text-xs hover:bg-accent/50 transition-colors"
